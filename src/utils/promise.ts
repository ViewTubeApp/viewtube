...
export function promiseAllProperties<T extends PlainObj>(promisesMap: PromisesMap<T>): Promise<T> {
  ...
  return Promise.all(promises).then((results) => reassembleResultsAsMap(keys, results) as T);
}
...
export function promiseAllSettledProperties<T extends PlainObj>(
  ...
  return Promise.allSettled(promises).then((results) => reassembleResultsAsMap(keys, results) as PromiseOutcomesMap<T>);
}
